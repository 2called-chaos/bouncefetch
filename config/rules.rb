...
Bouncefetch.rules do
  ...
  type :recipient_unknown do
    ...
    rule("address not present in directory")
    rule("address unknown")
    ...
    rule("no mailbox by that name is currently available")
    rule("no mailbox here by that name")
    rule("no recipients for yor mail here")
    rule("no such address")
    rule("no such mailbox")
    rule("no such user")
    rule("no such person")
    rule("not our customer")
    rule("not used")
